...
class TestAccountCreation:
    ...
    def test_create_account(self, client: TestClient, token: dict):
    ...
    def test_create_account_with_invalid_currency(self, client: TestClient, token: dict):
    ...
    def test_create_account_balance_below_zero(self, client: TestClient, token: dict):
...
class TestGetAccount:
    ...
    def test_get_account(self, client: TestClient, token: dict):
        ...
        response = client.get(f"/accounts/{account_id}", headers=token)
    ...
    def test_get_non_existent_account(self, client: TestClient, token: dict):
        # Intentar obtener una cuenta inexistente
        response = client.get("/accounts/99999", headers=token)
...
class TestListAccounts:
    ...
    def test_list_accounts(self, client: TestClient, token: dict):
...
class TestDeleteAccount:
    ...
    def test_delete_account(self, client: TestClient, token: dict):
        # Crear una cuenta primero
        response = client.post(
            "/accounts/",
            headers=token,
        ...
        response = client.delete(f"/accounts/{account_id}", headers=token)
    ...
    def test_delete_non_existent_account(self, client: TestClient, token: dict):
        # Intentar eliminar una cuenta que no existe
        response = client.delete("/accounts/99999", headers=token)
        assert response.status_code == 404
        assert response.json()["detail"] == "Account not found"
